...
- **Role‑based access**: Admin, Editor, and Author permissions backed by Supabase profiles.
...
## Role‑Based Access (Supabase)

NeoPress expects a `profiles` table with a `role` column to grant permissions.

**Roles**

- `Admin`: full access to create, edit, and delete posts.
- `Editor`: create, edit, and delete posts.
- `Author`: create/edit posts, delete disabled in the UI.
...
- **Real authentication**: expand Supabase Auth with password reset and MFA.
